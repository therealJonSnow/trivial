...
function statusOf(order: number, isScratch: boolean, view?: TimerView): RowStatus {
  ...
  return (
    <ul className={`divide-y divide-line ${className}`}>
      {schedule.starts.map((s) => {
        const status = statusOf(s.order, s.isScratch, view);
        return (
          <li
            key={s.order}
          ...
          >
            ...
            <span className="w-14 shrink-0 text-right font-mono text-sm tabular-nums text-ink">
              {formatMmSs(s.offsetMs)}
            </span>
            {firstGunEpoch !== undefined && (
